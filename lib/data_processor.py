...
class DataProcessor(object):
    ...
    def create_1hot_vectors(values, possible_values):
        ...
        onehot_values = np.zeros((num_values, len(possible_values)), dtype=np.float32)
    ...
    def __init__(self, word_to_vector, sentence_length, channels):
    ...
    def process_one(self, sentence):
        sample = np.zeros((self.sentence_length, self.word_dimension, self.channels))
        word_index = 0
        for word in sentence.split():
            ...
            try:
                ...
                continue
            sample[word_index] = word_embedding.reshape((self.word_dimension, self.channels))
            word_index += 1
        return sample

    def process(self, sentences):
        (sentences_count,) = sentences.shape
        samples = np.empty((sentences_count, self.sentence_length, self.word_dimension, self.channels), dtype=np.float32)
