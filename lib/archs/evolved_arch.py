...
class EvolvedArch(Arch):
    ...
    def build(self):
        ...
        for (layer1_filters, layer1_biases) in zip(self.layer1_list_filters, self.layer1_list_biases):
            ...
            layer1_pooling = tf.nn.max_pool(
                layer1_activation,
                # The whole conv out is max pooled
        ...
        layer2_linear = tf.matmul(
            layer1_dropout, self.layer2_weights) + self.layer2_biases
        ...
        layer3_linear = tf.matmul(
            layer2_dropout, self.layer3_weights) + self.layer3_biases

        self.prediction = tf.nn.softmax(layer3_linear)
        self.logits = layer3_linear
    ...
    def initialize_variables(self, session):
        ...
        session.run(self.layer3_weights.initializer)
        session.run(self.layer3_biases.initializer)

    def _get_variables_as_list(self):
        return self.layer1_list_filters + self.layer1_list_biases + [self.layer2_weights, self.layer2_biases, self.layer3_weights, self.layer3_biases]
